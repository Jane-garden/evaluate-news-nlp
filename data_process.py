...
# 保留了 ans_type 作为内部字段，用于后续 convert 判断，但不作为 feature 字段流出
def read_examples(full_file):
    ...
    for case in tqdm(full_data):
        ...
        if isinstance(raw_answer, list):
            if len(raw_answer) > 0 and isinstance(raw_answer[0], dict) and "text" in raw_answer[0]:
                orig_answer_text = raw_answer[0]["text"]
            else:
                orig_answer_text = ""
        else:
            orig_answer_text = raw_answer
        ...
        # 仅用于判断，不向 Feature 传
        answer_lower = str(orig_answer_text).lower()
        if answer_lower == 'yes':
            ans_type = 1
        elif answer_lower == 'no':
            ans_type = 2
        elif answer_lower == 'unknown':
            ans_type = 3
        else:
            ans_type = 0

        JUDGE_FLAG = ans_type in [1, 2, 3]
        FIND_FLAG = False
        ...
        if not JUDGE_FLAG and len(sup_facts_sent_id) > 0:
            sp_sent_idx = sup_facts_sent_id[0]
            sp_start, sp_end = sent_start_end_position[sp_sent_idx]
            ans_start_position = [sp_start]
            ans_end_position = [sp_end]
            FIND_FLAG = True
        ...
        # ✅ 保留 ans_type 作为 example 的动态字段（不在 __init__ 中定义）
        example.ans_type = ans_type
...
#         raw_answer = case.get("answer", "")   ### 修改3：统一处理 answer 字段可能为字符串 / 列表
#         if isinstance(raw_answer, list):
#             # 如果是 [{"text": ..., "answer_start": ...}] 结构
#             if len(raw_answer) > 0 and isinstance(raw_answer[0], dict) and "text" in raw_answer[0]:
#                 orig_answer_text = raw_answer[0]["text"]
#             else:
#                 orig_answer_text = ""
#         else:
#             orig_answer_text = raw_answer
#         # orig_answer_text = case['answer']   ### 修改3：原始 answer 有时为 list，提前统一
#         # if isinstance(orig_answer_text, list) and len(orig_answer_text) > 0:
#         #     orig_answer_text = orig_answer_text[0]
...
#         JUDGE_FLAG = str(orig_answer_text).lower() in ['yes', 'no', 'unknown']
#         FIND_FLAG = False
...
#                 tmp_answer = orig_answer_text if isinstance(orig_answer_text, str) else " ".join(orig_answer_text)  ### 修改4：拼接 answer 为字符串
...
#             para_start_end_position.append((para_start_position, para_end_position, title, is_gold_para))
#             # import random
#             # if not JUDGE_FLAG and orig_answer_text.strip():
#             #     sample_sents = random.sample(sents, max(len(sents), 10))
#             #     print(f"\n【答案匹配检查 - qid {key}】答案文本：{orig_answer_text}")
#             #     for sent in sample_sents:
#             #         if orig_answer_text in sent:
#             #             print("✅ 答案在句子中找到了")
#         else:
#             print("❌ 答案不在句子中：", sent)
...
if __name__ == '__main__':
    ...
    tokenizer = BertTokenizer.from_pretrained(args.tokenizer_path, local_files_only=True)
    # tokenizer= AutoTokenizer.from_pretrained(args.tokenizer_path) #加入新模型
