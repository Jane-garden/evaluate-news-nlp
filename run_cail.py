...
# 在 run_cail.py 文件开头，定义函数，不会有任何副作用：
import matplotlib.pyplot as plt
import os

def plot_step_losses(losses2, losses3, save_path="output/loss_by_step.png"):
    steps = list(range(1, len(losses2) + 1))
    plt.figure(figsize=(10, 6))
    plt.plot(steps, losses2, marker='o', label="Loss2 - Type")
    plt.plot(steps, losses3, marker='x', label="Loss3 - Support")
...
def predict(model, dataloader, example_dict, feature_dict, prediction_file, need_sp_logit_file=False):
    ...
    def plot_metrics(losses, save_path="output/metrics_summary.png"):
        ...
        plt.title("Training Summary: Test Loss ")
        plt.xlabel("Epoch")
...
def train_batch(model, batch):
    global global_step, total_train_loss,step_losses2, step_losses3
    ...
    # ✅ 每步记录 loss2，3
    step_losses2.append(loss_list[2].item())
    step_losses3.append(loss_list[3].item())
...
if __name__ == "__main__":
    ...
    step_losses3 = []        # ✅ 每步记录 loss3
    step_losses2 = []       # ✅ 每步记录 loss2 (type loss)
    
    while True:
        if epc == args.epochs:  # 5 + 30
            timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
            save_path = f"output/metrics_steps_{timestamp}.png"
            plot_step_losses(step_losses2, step_losses3, save_path=save_path)
            print(f"\n✅ 训练完成，step 曲线图已保存：{save_path}")
            exit(0)
        epc += 1
